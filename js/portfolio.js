'use strict';

var portData = [
  {
    title: 'Campfire Coffee',
    projectUrl: 'http://rstoner19.github.io/campfire-coffee',
    category: '201 CodeFellows',
    createdOn: '2016-01-15',
    detail: 'A project in CodeFellows 201 that required an output based on a random value based on a minimum and maximum input, as well as allowing the user to input data for a new coffe location.'
  },
  {
    title: 'Lorem Ipsum',
    projectUrl: 'google.com',
    category: 'Lorem',
    createdOn: '2014-01-15',
    detail: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse ac eros posuere, viverra quam nec, euismod mauris. Mauris risus velit, sagittis ac accumsan tempor, pretium sit amet nisl. Integer posuere porttitor dolor. Phasellus ac risus bibendum, vehicula urna non, fermentum metus. Quisque varius egestas turpis ut gravida. Cras sed iaculis sem. Suspendisse ultrices fermentum eros, vitae tempor tellus porta eget. Morbi posuere est at nunc lobortis placerat. Cras dictum augue sit amet ante blandit, placerat sodales nisi tincidunt. Maecenas id sem tincidunt, sodales lorem eget, pretium justo. Sed quis molestie ante. Quisque nec mauris volutpat, mattis elit sed, ornare nulla. Morbi imperdiet egestas ligula nec tempus. Cras aliquam et diam ut faucibus. Lorem ipsum dolor sit amet, consectetur adipiscing elit.'
  },
  {
    title: 'Cliquor Cabinet',
    projectUrl: 'http://pasaunders.github.io/cocktail-project/index.html',
    category: '201 CodeFellows',
    image: 'img/cliquor.jpg',
    createdOn: '2016-01-29',
    detail: 'A group project where we created a list of drinks based on a user\'s ingredients that are stored on local storage.  If the user does put in a drink, it\'ll be added to a database.  Once the user has their ingredients in, they can display drinks in the databased that are sorted based on what percentage of ingredients in the drink they have.  Additionally, it can be sorted by based liqour or various categories.'
  }
];

var aboutMe = {
  fullName: 'Rick Stoner',
  college: 'University of Utah',
  major: 'Finance',
  bio: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aenean luctus purus id purus fermentum, id interdum arcu blandit. Ut non eleifend nunc. Vivamus accumsan vulputate vehicula. Etiam viverra consequat egestas. Etiam quis leo metus. Nullam vestibulum tellus vehicula, malesuada nunc quis, tristique tellus. Praesent varius viverra fermentum. Proin sit amet varius ante. Duis molestie egestas tellus sit amet ultrices. Etiam malesuada arcu ipsum, id blandit neque dignissim nec. Integer lobortis commodo dictum. Integer id malesuada lacus. Nulla facilisi. Aliquam condimentum vehicula dolor et condimentum. Integer eget vulputate nulla.'
};
